...
//	FAnimationRuntime::ConvertBoneSpaceTransformToCS(Output.AnimInstanceProxy->GetComponentTransform(), Output.Pose, BS_OG_EndBoneCSTransform, IKBoneCompactPoseIndex, EBoneControlSpace::BCS_ComponentSpace);
	...
	UDragonIK_Library::Solve_Modified_Direct_TwoBoneIK_4(custom_arm_sizes,mesh_component_transform, wrist_offset,UpperLimbCSTransform, LowerLimbCSTransform, EndBoneCSTransform, JointTargetPos, DesiredPos, bAllowHandStretching,1, HandData.Max_Stretch_Ratio, clamped_stretch_priority *2,up_arm_twist_technique);
	...
	*/
	...
	// Update transform for end bone.
	{
		...
		//	FQuat Hand_Rotation_Temporary = ThighTransform.GetRotation();
			FQuat Hand_Rotation_Temporary = mesh_component_transform.GetRotation().Inverse() * ThighTransform.GetRotation();
		...
		//	EndBoneCSTransform.SetRotation(Default_Rot);
			...
			input_arm_rot = GetHandYaw(true,HandData,mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform, Unmodifed_Hand,Up_Vector_Val);
...
//	FRotator input_arm_rot = GetHandYaw(mesh_component_transform,Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform);	
...
//	input_arm_rot.Roll = UKismetMathLibrary::Lerp(0, ( input_arm_rot.Roll), roll_alpha);
	...
	*/
	...
	BS_LowerLimbTransform = SetArmYaw(HandData.invert_lower_twist,HandData.is_this_right_hand,input_arm_rot.Roll,mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), Output.Pose.GetComponentSpaceTransform(IKBoneCompactPoseIndex), LowerLimbCSTransform, EndBoneCSTransform);
...
//	BS_UpperLimbTransform = SetArmYaw(HandData.is_this_right_hand, input_arm_rot.Roll, mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), UpperLimbCSTransform, LowerLimbCSTransform);
...
//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " forearm_angle " + FString::SanitizeFloat(forearm_angle));
	...
	FRotator Forearm_Input_Rot = GetHandYaw(false,HandData,mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), BS_UpperLimbTransform, BS_LowerLimbTransform, BS_LowerLimbTransform, Up_Vector_Val);
...
//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " Forearm_Input_Rot.Roll " + FString::SanitizeFloat(Forearm_Input_Rot.Roll)+"  original_rot : "+ FString::SanitizeFloat(original_rot));
	...
	if (shoulder_twist_available)
	{
		...
		//ShoulderTwistBone = SetArmYaw(HandData.is_this_right_hand, last_shoulder_angle * Arm_Angle, mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedUpperTwistLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), ShoulderTwistBone, BS_LowerLimbTransform);

		BS_UpperLimbTransform = SetArmYaw(HandData.invert_upper_twist, HandData.is_this_right_hand, last_shoulder_angle * Arm_Angle, mesh_component_transform, Output.Pose.GetComponentSpaceTransform(CachedUpperLimbIndex), Output.Pose.GetComponentSpaceTransform(CachedLowerLimbIndex), BS_UpperLimbTransform, BS_LowerLimbTransform);
