/**
* Modified version of griffin handle map to work on Unreal Engine
* @file handle_map-inl.h
* @author Jeff Kiah
* @copyright The MIT License (MIT), Copyright (c) 2015 Jeff Kiah
*/
#pragma once
#ifndef GRIFFIN2_HANDLE_MAP_INL_H_
#define GRIFFIN2_HANDLE_MAP_INL_H_
...
namespace griffin2 {
	...
	{
		IdSet_T handles(n);
		check(n > 0 && "emplaceItems called with n = 0");
		m_fragmented = 1;
	...
	{
		check(handle.index < m_sparseIds.size() && "outer index out of range");

		Id_T innerId = m_sparseIds[handle.index];

		check(handle.typeId == m_itemTypeId && "typeId mismatch");
		check(handle.generation == innerId.generation && "at called with old generation");
		check(innerId.index < m_items.size() && "inner index out of range");
